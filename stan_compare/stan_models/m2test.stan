functions{
  ...
  real CAR_positive_FOB(real time){
   ...
   array[] real ODE_sys(real time,  array[] real y, array[] real parms, array[] real rdata, array[] int idata) {
     ...
     dydt[2] = beta * Total_FoB(time)  - lambda * y[2];
...
transformed parameters{
  array[numObs1, 2] real y_hat;     // declaring the array for ODE solution

  array[numObs1] real CAR_GCcounts_mean;
  array[numObs1] real CAR_MZcounts_mean;

  array[4] real parms;                  // declaring the array for parameters
  array[2] real init_cond;              // declaring the array for state variables
